...
tag_to_version() {
  if [ -z "${VERSION}" ]; then
    ...
    log_info "checking GitHub for tag '${VERSION}'"
  fi
  REALTAG=$(github_release "$OWNER/$REPO" "${VERSION}") && true
  if test -z "$REALTAG"; then
    log_crit "unable to find '${VERSION}' - use 'latest' or see https://github.com/${PREFIX}/releases for details"
    exit 1
  fi
...
log_info "found version: ${VERSION_STR} for ${VERSION}/${OS}/${ARCH}"

NAME=${PROJECT_NAME}_${VERSION_STR}_${OS}_${ARCH}
TARBALL=${NAME}.${FORMAT}
TARBALL_URL=${GITHUB_DOWNLOAD}/${VERSION}/${TARBALL}
CHECKSUM=${PROJECT_NAME}_${VERSION_STR}_checksums.txt
